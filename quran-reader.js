...
const QuranReader = {
    pdf: null,
    pdfParts: [],  // [{pdf, startPage, endPage}, ...]
    totalPages: 0,
    currentPage: 1,
    scale: 1.5,
    isOpen: false,
    ...
    // PDF URLs (split parts only — full PDF removed to save space)
    PDF_PARTS: [
        'assets/quran_ar_de_v2_part1.pdf',
        'assets/quran_ar_de_v2_part2.pdf',
        'assets/quran_ar_de_v2_part3.pdf'
    ],

    /** Get page from correct split part */
    async _getPage(globalPageNum) {
        if (this.pdf) return this.pdf.getPage(globalPageNum);
        for (const part of this.pdfParts) {
            if (globalPageNum >= part.startPage && globalPageNum <= part.endPage) {
                return part.pdf.getPage(globalPageNum - part.startPage + 1);
            }
        }
    ...
    async open() {
        ...
        try {
            if (!this.pdf && this.pdfParts.length === 0) {
                // Load split PDF parts
                this.pdfParts = [];
                let pageOffset = 1;
                for (let i = 0; i < this.PDF_PARTS.length; i++) {
                    this.showLoading('📖 Teil ' + (i + 1) + '/' + this.PDF_PARTS.length + ' wird geladen...');
                    const resp = await fetch(this.PDF_PARTS[i]);
                    if (!resp.ok) throw new Error('Part ' + (i + 1) + ' failed: ' + resp.status);
                    const data = await resp.arrayBuffer();
                    const partPdf = await pdfjsLib.getDocument({ data }).promise;
                    const n = partPdf.numPages;
                    this.pdfParts.push({ pdf: partPdf, startPage: pageOffset, endPage: pageOffset + n - 1 });
                    pageOffset += n;
                    console.log('✅ Part ' + (i + 1) + ':', n + ' pages');
                }
                this.totalPages = pageOffset - 1;
                console.log('✅ All parts loaded:', this.totalPages, 'pages');
                this._sanityCheck();
            }
            ...
            this.showError('📖 Quran-PDF nicht gefunden. Bitte PDF-Dateien im assets/ Ordner prüfen.');
        }
    },
