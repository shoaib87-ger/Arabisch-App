...
const EbookUI = {
    ...
    /** Go back: close panels first, then reading to library, library to close */
    back() {
        ...
        // Close TOC overlay if open
        const toc = document.getElementById('ebookTocOverlay');
        if (toc && toc.classList.contains('active')) {
            this.closeTOC();
            return;
        }
        // Close go-to-page input if visible
        const gotoEl = document.getElementById('ebookGoto');
        if (gotoEl && gotoEl.style.display === 'flex') {
    ...
    async handleFileSelect(event) {
        ...
        // Save to IndexedDB — convert File to ArrayBuffer (iOS-safe)
        const id = 'book_' + Date.now() + '_' + Math.random().toString(36).substr(2, 6);
        let fileData;
        try {
            fileData = await file.arrayBuffer();
        } catch (abErr) {
            console.error('ArrayBuffer conversion error:', abErr);
            alert('Fehler beim Lesen der Datei: ' + abErr.message);
            return;
        }
        ...
        const bookObj = {
            ...
            type: file.type || detected.mime || 'application/octet-stream',
            data: fileData,  // ArrayBuffer, NOT File/Blob
            lastLocation: null,
            addedAt: Date.now()
        };
    ...
    async openBook(id) {
        ...
        try {
            // Backwards compat: old entries have book.blob (Blob/File), new entries have book.data (ArrayBuffer)
            let fileBlob;
            if (book.data && (book.data instanceof ArrayBuffer || ArrayBuffer.isView(book.data))) {
                fileBlob = new Blob([book.data], { type: book.type || 'application/octet-stream' });
            } else if (book.blob) {
                fileBlob = book.blob;
            } else {
                throw new Error('Keine Buchdaten gefunden');
            }
            await this.activeViewer.open(fileBlob, id, book.lastLocation);
    ...
    _setupReaderShell(book) {
        ...
        header.innerHTML = `
            ...
            <div style="display:flex;gap:6px;">
                <button class="ebook-settings-btn" onclick="EbookUI.showTOC()" title="Inhaltsverzeichnis">📑</button>
                <button class="ebook-settings-btn" onclick="EbookUI.toggleSettings()">⚙️</button>
            </div>
        `;

        // Footer with nav + progress (clickable progress toggles GoTo)
        footer.innerHTML = `
    ...
    _renderSettings() {
        ...
        panel.innerHTML = `
            <div class="ebook-settings-content">
                <div class="ebook-settings-header">
                    <h4>Einstellungen</h4>
                    <button class="ebook-settings-close" onclick="EbookUI.toggleSettings()">✕</button>
                </div>
    ...
    /** Show/hide loading indicator on GoTo (while EPUB locations generate) */
    _setGoToLoading(loading) {
        const loader = document.getElementById('ebookGoToLoader');
        if (loader) loader.style.display = loading ? 'inline' : 'none';
        const btn = document.querySelector('.ebook-goto-btn');
    ...
    /** Show TOC modal (for EPUB and any viewer with getTOC) */
    async showTOC() {
        if (!this.activeViewer || typeof this.activeViewer.getTOC !== 'function') {
            // For PDF/Text: show go-to-page input instead
            this._toggleGoTo();
            return;
        }

        const toc = await this.activeViewer.getTOC();
        if (!toc || toc.length === 0) {
            // No TOC available — show go-to input
            this._toggleGoTo();
            return;
        }

        // Show TOC overlay
        let tocOverlay = document.getElementById('ebookTocOverlay');
        if (!tocOverlay) {
            tocOverlay = document.createElement('div');
            tocOverlay.id = 'ebookTocOverlay';
            tocOverlay.className = 'ebook-toc-overlay';
            document.getElementById('ebookOverlay').appendChild(tocOverlay);
        }

        tocOverlay.innerHTML = `
            <div class="ebook-toc-panel">
                <div class="ebook-toc-header">
                    <h4>📑 Inhaltsverzeichnis</h4>
                    <button class="ebook-toc-close" onclick="EbookUI.closeTOC()">✕</button>
                </div>
                <div class="ebook-toc-list">
                    ${toc.map(ch => `
                        <button class="ebook-toc-item" onclick="EbookUI.goToChapter('${ch.href}')">
                            ${this._escapeHtml(ch.label)}
                        </button>
                    `).join('')}
                </div>
            </div>
    ...
    closeTOC() {
        const tocOverlay = document.getElementById('ebookTocOverlay');
        if (tocOverlay) tocOverlay.classList.remove('active');
    },

    async goToChapter(href) {
        this.closeTOC();
        if (this.activeViewer && typeof this.activeViewer.goToChapter === 'function') {
            await this.activeViewer.goToChapter(href);
        }
    },

    /** Go to specific page/location number */
    goToPage() {
        const input = document.getElementById('ebookGotoInput');
        if (!input) return;
        ...
        if (this.activeViewer && typeof this.activeViewer.goToPage === 'function') {
